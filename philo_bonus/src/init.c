...
/*   Updated: 2023/02/19 12:51:44 by rertzer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ph_init_phdata(t_phdata *phdata, int argc, char **argv)
{
	...
	if (0 == phdata->nb_of_philo)
		ph_exit_parsing("zero philosospher");
	...
	if (6 == argc)
	{
		...
		if (0 == phdata->must_eat)
			ph_exit_parsing("zero lunch");
	}
	else
		phdata->must_eat = -1;
...
void	ph_init_semaphores(t_phdata *phdata)
{
	...
	phdata->alife = ph_semaphore_open(phdata, ALIFE_NAME, 1);
